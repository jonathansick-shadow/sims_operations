...
def report(hostname, sessionID, notcs):
    ...
    label = 'session info'
    sql = 'select sessionHost, sessionDate from tSession_%s_%d where sessionID=%d' % (
        hostname, sessionID, sessionID)
    ...
    label = 'config'
    sql = 'select paramName,paramValue from tConfig_%s_%d where Session_sessionID=%d and moduleName="LSST" order by paramIndex' % (
        hostname, sessionID, sessionID)
...
#    print dict_LSST
    ...
    sql = 'select count(event) from tTimeHistory_%s_%d where Session_sessionID=%d and event=%d' % (
        hostname, sessionID, sessionID, START_NIGHT)
    ret = getDbData(sql, label)
    count_nights = ret[0][0]
    print "%s: %s" % (label, count_nights)

    label = 'number of visits'
    sql = 'select count(*) from tSlewHistory_%s_%d where ObsHistory_Session_sessionID=%d' % (hostname,
                                                                                             sessionID, sessionID)
    ret = getDbData(sql, label)
    ...
    sql = 'select avg(visitExpTime) from tObsHistory_%s_%d where Session_sessionID=%d' % (
        hostname, sessionID, sessionID)
    ret = getDbData(sql, label)
    ...
    sql = 'select avg(visitTime) from tObsHistory_%s_%d where Session_sessionID=%d' % (
        hostname, sessionID, sessionID)
    ret = getDbData(sql, label)
    avg_visittime = ret[0][0]
    ...
    sql = 'select avg(slewTime) from tSlewHistory_%s_%d where ObsHistory_Session_sessionID=%d' % (
        hostname, sessionID, sessionID)
    ret = getDbData(sql, label)
    ...
    sql = 'select propID,propConf,propName from tProposal_%s_%d where Session_sessionID=%d' % (
        hostname, sessionID, sessionID)
    ret = getDbData(sql, label)
    list_proposals = ret
    ...
    for filter in ['u', 'g', 'r', 'i', 'z', 'y']:
        ...
        sql = 'select count(*) from tObsHistory_%s_%d where Session_sessionID=%d and filter="%s"' % (
            hostname, sessionID, sessionID, filter)
        ret = getDbData(sql, label)
    ...
    for proposal in list_proposals:
        ...
        sql = 'select count(*) from tObsHistory_Proposal_%s_%d where Proposal_propID=%d' % (hostname,
                                                                                            sessionID, propID)
        ret = getDbData(sql, label)
        numVisits = ret[0][0]
        ...
        for filter in ['u', 'g', 'r', 'i', 'z', 'y']:
            ...
            sql = 'select count(*) from tObsHistory_%s_%d oh INNER JOIN tObsHistory_Proposal_%s_%d ohp ON oh.obsHistID=ohp.ObsHistory_obsHistID where ohp.Proposal_propID=%d and oh.filter="%s" and oh.Session_sessionID=%d' % (
                hostname, sessionID, hostname, sessionID, propID, filter, sessionID)
            ret = getDbData(sql, label)
...
def reportAngleStats(name, hostname, sessionID):
    ...
    sql = 'select min(%s),max(%s),avg(%s),std(%s) from tSlewState_%s_%d join tSlewHistory_%s_%d where tSlewState_%s_%d.SlewHistory_slewID=tSlewHistory_%s_%d.slewID and ObsHistory_Session_sessionID=%d and state=1' % (
        name, name, name, name, hostname, sessionID, hostname, sessionID, hostname, sessionID, hostname, sessionID, sessionID)
    ret = getDbData(sql, label)
...
def reportActivity(name, hostname, sessionID, total):
    ...
    sql = 'select avg(actDelay),max(actDelay),count(actDelay) from tSlewActivities_%s_%d join tSlewHistory_%s_%d where tSlewActivities_%s_%d.SlewHistory_slewID=tSlewHistory_%s_%d.slewID and ObsHistory_Session_sessionID=%d and activity="%s" and actDelay>0' % (
        hostname, sessionID, hostname, sessionID, hostname, sessionID, hostname, sessionID, sessionID, name)
    ret = getDbData(sql, label)
    A = ret[0][0]
    M = ret[0][1]
    N = ret[0][2]
    sql = 'select count(*),avg(actDelay) from tSlewActivities_%s_%d join tSlewHistory_%s_%d where tSlewActivities_%s_%d.SlewHistory_slewID=tSlewHistory_%s_%d.slewID and ObsHistory_Session_sessionID=%d and activity="%s" and inCriticalPath="True" and actDelay>0' % (
        hostname, sessionID, hostname, sessionID, hostname, sessionID, hostname, sessionID, sessionID, name)
    ret = getDbData(sql, label)
    C = ret[0][0]
    CA = ret[0][1]
...
def reportMaxSpeed(name, hostname, sessionID):
    ...
    sql = 'select avg(abs(%sSpd)),max(abs(%sSpd)),max(slewCount) from tSlewMaxSpeeds_%s_%d join tSlewHistory_%s_%d where tSlewMaxSpeeds_%s_%d.SlewHistory_slewID=tSlewHistory_%s_%d.slewID and ObsHistory_Session_sessionID=%d' % (
        name, name, hostname, sessionID, hostname, sessionID, hostname, sessionID, hostname, sessionID, sessionID)
    ret = getDbData(sql, label)
    A = ret[0][0]
    M = ret[0][1]
    T = ret[0][2]
    sql = 'select count(*) from tSlewMaxSpeeds_%s_%d join tSlewHistory_%s_%d where tSlewMaxSpeeds_%s_%d.SlewHistory_slewID=tSlewHistory_%s_%d.slewID and ObsHistory_Session_sessionID=%d and abs(%sSpd)>=%f' % (
        hostname, sessionID, hostname, sessionID, hostname, sessionID, hostname, sessionID, sessionID, name, 0.99*M)
    ret = getDbData(sql, label)
    C = ret[0][0]
...
def reportSequences(label, propID):
    ...
    sql = 'select count(*) from tSeqHistory_%s_%d where sessionID=%d and propID=%d and endStatus=0' % (
        hostname, sessionID, sessionID, propID)
    ret = getDbData(sql, label)
    N_complete = ret[0][0]

    sql = 'select count(*) from tSeqHistory_%s_%d where sessionID=%d and propID=%d and endStatus=1' % (
        hostname, sessionID, sessionID, propID)
    ret = getDbData(sql, label)
    N_lost_missed = ret[0][0]

    sql = 'select count(*) from tSeqHistory_%s_%d where sessionID=%d and propID=%d and endStatus=3 and completion>0' % (
        hostname, sessionID, sessionID, propID)
    ret = getDbData(sql, label)
    ...
    sql = 'select completion,count(0) from tSeqHistory_%s_%d where sessionID=%d and propID=%d group by completion' % (
        hostname, sessionID, sessionID, propID)
    ret = getDbData(sql, label)
...
def reportWL(hostname, sessionID):
    ...
    label = 'ids_WL'
    sql = 'select propID,propConf from tProposal_%s_%d where Session_sessionID=%d and propName="WL"' % (
        hostname, sessionID, sessionID)
    ...
    for q in range(len(ids_WL)):
        ...
        propConf = ids_WL[q][1]
        label = 'Weak Lensing'
        sql = 'select paramName,paramValue from tConfig_%s_%d where Session_sessionID=%d and moduleName="weakLensing" and nonPropID=%d order by paramIndex' % (
            hostname, sessionID, sessionID, propID)
        ...
        sql = 'select count(Field_fieldID) from tProposal_Field_%s_%d where Proposal_propID=%d;' % (
            hostname, sessionID, propID)
        ret = getDbData(sql, label)
    ...
    while(False):
        ...
        sql = 'create table tmpObs select propID,fieldID,fieldRA,fieldDec,filter,expDate from tObsHistory_%s_%d where sessionID=%d and propID=%d group by expDate order by propID,fieldID,filter' % (
            hostname, sessionID, sessionID, propID)
        ret = getDbData(sql, label)
...
# print "%s: histogram %s %6.2f%% %d" % (propConf, filter,
# H[filter][p][0]*100.0/Filter_Visits[filter], H[filter][p][1])
...
def reportWLTSS(sessionID, host):
    ...
    sql = 'select propID,propConf from tProposal where Session_sessionID=%d and propName="WLTSS"' % (
        sessionID)
    ...
    for p in range(len(list_propID)):
        ...
        sql = 'select paramValue from tConfig where Session_sessionID=%d and nonpropID=%d and paramName="SubSeqFilters" order by paramIndex' % (
            sessionID, propID)
        list_filter = getDbData(sql, label)

        sql = 'select paramValue from tConfig where Session_sessionID=%d and nonpropID=%d and paramName="SubSeqEvents" order by paramIndex' % (
            sessionID, propID)
        list_events = getDbData(sql, label)

        sql = 'select paramValue from tConfig where Session_sessionID=%d and nonpropID=%d and paramName="SubSeqInterval" order by paramIndex' % (
            sessionID, propID)
        list_interval = getDbData(sql, label)

        sql = 'select paramValue from tConfig where Session_sessionID=%d and nonpropID=%d and paramName="SubSeqWindowStart" order by paramIndex' % (
            sessionID, propID)
        list_windowstart = getDbData(sql, label)

        sql = 'select paramValue from tConfig where Session_sessionID=%d and nonpropID=%d and paramName="SubSeqWindowEnd" order by paramIndex' % (
            sessionID, propID)
        ...
        for f in range(len(list_filter)):
            ...
            ret = getDbData(sql, label)
            sql = 'create table tmpWLTSSvisits select seqnNum,count(*) as visits from ObsHistory where sessionID=%d and propID=%d and filter="%s" group by seqnNum' % (
            ...
            sql = 'select seqnNum,expDate,fieldID,fieldRA,fieldDec,expMJD from ObsHistory where sessionID=%d and propID=%d and filter="%s" order by seqnNum,expDate' % (
                sessionID, propID, filter)
            ...
            if interval > 0:
                ...
                #	        histogram = getDbData(sql, label)
                #	        for h in range(len(histogram)):
                # print "%s: pairs histogram filter=%s %d %d" % (propConf, filter,
                # histogram[h][0], histogram[h][1])

                sql = 'DROP TABLE IF EXISTS tmpWLTSSpairs'
                ret = getDbData(sql, label)
                sql = 'create table tmpWLTSSpairs select seqnNum,floor(count(0)/2) as maxpairs from ObsHistory where sessionID=%d and propID=%d and filter="%s" group by seqnNum' % (
                    sessionID, propID, filter)
                ret = getDbData(sql, label)
