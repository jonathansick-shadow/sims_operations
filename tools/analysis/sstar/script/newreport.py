...
def report(database, sessionID, notcs):
    ...
    label = 'config'
    sql = 'select paramName,paramValue from Config where Session_sessionID=%d and moduleName="LSST" order by paramIndex' % (
        sessionID)
...
#    print dict_LSST
    ...
    sql = 'select count(event) from TimeHistory where Session_sessionID=%d and event=%d' % (
        sessionID, START_NIGHT)
    ret = getDbData(database, sql)
    ...
    sql = 'select avg(t.expTime) from (select expDate,expTime from ObsHistory where Session_sessionID=%d group by expDate) as t' % (
        sessionID)
    ret = getDbData(database, sql)
    ...
    sql = 'select avg(t.slewTime) from (select expDate,slewTime from ObsHistory where Session_sessionID=%d group by expDate) as t' % (
        sessionID)
    ret = getDbData(database, sql)
...
def reportAngleStats(name, sessionID):
    ...
    sql = 'select min(%s),max(%s),avg(%s),std(%s) from SlewFinalState where sessionID=%d' % (
        name, name, name, name, sessionID)
    ret = getDbData(database, sql)
...
def reportActivity(name, sessionID, total):
    ...
    sql = 'select avg(delay),max(delay),count(delay) from SlewActivities where sessionID=%d and activity="%s" and delay>0' % (
        sessionID, name)
    ret = getDbData(database, sql)
    A = ret[0][0]
    M = ret[0][1]
...
def reportMaxSpeed(name, sessionID):
    ...
    sql = 'select avg(abs(%sSpd)),max(abs(%sSpd)),max(slewCount) from SlewMaxSpeeds where sessionID=%d' % (
        name, name, sessionID)
    ret = getDbData(database, sql)
    A = ret[0][0]
    M = ret[0][1]
...
def reportSequences(label, propID):
    ...
    sql = 'select completion,count(0) from SeqHistory where sessionID=%d and propID=%d group by completion' % (
        sessionID, propID)
    ret = getDbData(database, sql)
...
def reportWL(sessionID):
    ...
    for q in range(len(ids_WL)):
        ...
        propConf = ids_WL[q][1]
        label = 'Weak Lensing'
        sql = 'select paramName,paramValue from Config where sessionID=%d and moduleName="weakLensing" and propID=%d order by paramIndex' % (
            sessionID, propID)
