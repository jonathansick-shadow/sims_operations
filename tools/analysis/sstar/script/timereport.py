...
#    SEEINGTABLE = getDbData(sql)[0][0]
    ...
    sql = 'select expStartTime,expDelay,cloudiness from NOBHist where sessionID=%d order by expStartTime' % (
        sessionID)
    ...
    for k in range(len(filterList)):
        ...
        sql = 'select expDate,slewTime,expTime,rawSeeing,count(*),propID from ObsHistory where sessionID=%d and filter="%s" group by expDate' % (
            sessionID, filter)
