...
def reportAngleStats(name, sessionID):
    ...
    sql = 'select min(%s),max(%s),avg(%s),std(%s) from SlewState join SlewHistory where SlewState.SlewHistory_slewID=SlewHistory.slewID and ObsHistory_Session_sessionID=%d' % (
...
def reportMaxSpeed(name, sessionID):
    ...
    sql = 'select avg(abs(%sSpd)),max(abs(%sSpd)),max(slewCount) from SlewMaxSpeeds join SlewHistory where SlewMaxSpeeds.SlewHistory_slewID=SlewHistory.slewID and ObsHistory_Session_sessionID=%d' % (
        name, name, sessionID)
    ret = getDbData(sql, label)
    A = ret[0][0]
    M = ret[0][1]
...
def reportWL(sessionID):
    ...
    for q in range(len(ids_WL)):
        ...
        propConf = ids_WL[q][1]
        label = 'Weak Lensing'
        sql = 'select paramName,paramValue from Config where Session_sessionID=%d and moduleName="weakLensing" and nonPropID=%d order by paramIndex' % (
            sessionID, propID)
