...
def seqReport(propID):
    ...
    sql = 'select sequenceID,fieldID,startDate,actualEvents from SeqHistory where propID=%d and endStatus=1' % (
        propID)
    print sql
    ...
    sql = 'select seqnNum,fieldID,filter,expDate,count(0) from ObsHistory where propID=%d group by seqnNum,filter' % (
        propID)
    print sql
    ret = getDbData(sql, label)
