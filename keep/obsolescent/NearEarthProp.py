...
class NearEarthProp (TransientProp):
    ...
    def startNight(self, dateProfile, moonProfile, startNewLunation, randomizeSequencesSelection, nRun, mountedFiltersList):
        ...
        super(NearEarthProp, self).startNight(dateProfile, moonProfile,
                                              startNewLunation, randomizeSequencesSelection, nRun, mountedFiltersList)
        ...
        if (self.log):
            self.log.info('NearEarthProp: startNight() propID=%d date=%.0f moon phase=%.2f' %
                          (self.propID, date, self.currentMoonPhase))
...
#	    for fieldID in self.tonightTargets.keys():
            ...
            if (self.log and removedsequences > 0):
                self.log.info('NearEarthProp: startNight() REMOVED %d idle sequences due to insufficient time to complete' % (
                    removedsequences))
...
#                    for filter in rankForFilters.keys():
                    for filter in sorted(rankForFilters.iterkeys()):
                        ...
                        # print "suggestList field=%d filter=%s rank=%f solarElong_DEG = %f" %
                        # (fieldID, filter, rank, solarElong_DEG)
...
#            self.reuseRanking = self.reuseRankingCount
    ...
    def CheckObservingCycle(self, date):
        ...
        if (self.currentMoonPhase > self.maxMoonPhase):
            if self.log and (self.verbose > 1):
                self.log.info('NearEarthProp: suggestObs() propID=%d moon too bright = %.2f' %
                              (self.propID, self.currentMoonPhase))
    ...
    def RankFilters(self, fieldID, filterSeeingList, allowedFilterList):
        ...
        for filter in allowedFilterList:
            if (filterSeeingList[filter] > self.maxSeeing):
                if self.log and self.verbose > 1:
                    self.log.info('NearEarthProp: suggestObs() propID=%d field %i bad seeing:%f' %
