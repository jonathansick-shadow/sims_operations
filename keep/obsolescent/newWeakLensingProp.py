...
class WeakLensingProp (Proposal):
    ...
    def startNight(self, dateProfile, moonProfile, startNewLunation, randomizeSequencesSelection, nRun, mountedFiltersList):
        ...
        super(WeakLensingProp, self).startNight(dateProfile,
                                                moonProfile, startNewLunation, mountedFiltersList)
    ...
    def GetProgressPerFilter(self):
        ...
        for filter in filters:
            if self.GoalVisitsFieldFilter[filter] > 0:
                if len(allfields) > 0:
                    progressFilter[filter] = float(coaddedFilterProgress[filter]) / \
                        len(allfields)/self.GoalVisitsFieldFilter[filter]
                else:
...
#                                    self.sessionID, fieldID=None)
        ...
        if needTonight > 0:
            ...
            GlobalNeedFactor = (self.maxNeedAfterOverflow/(self.VisitsTonight -
                                                           self.GoalVisitsTonight+1)) / self.GoalVisitsTonight
        ...
        for fieldID in listOfFieldsToEvaluate:
            ...
            if self.ProgressToStartBoost < progress_avg < 1.0:
                FieldNeedFactor += self.MaxBoostToComplete * \
                    (progress_avg-self.ProgressToStartBoost)/(1.0-self.ProgressToStartBoost)
            ...
            # print ".....field:%d ra:%f dec:%f am:%f phs:%f brite:%f" % (fieldID,
            # ra,dec, airmass, moonPhase, intargetProfiles[i][1]) , filterSeeingList
            ...
            for filter in allowedFilterList:
                ...
                if filterSeeingList[filter] > self.FilterMaxSeeing[filter]:
                    ffilter_badseeing += 1
                    # DBGprint "TOSS: fld:%d fltr:%s ra:%f dec%f airms:%f phs:%f  airAdjSee:%f
                    # > maxSee:%f" %
                    # (fieldID,filter,ra,dec,airmass,moonPhase,filterSeeingList[filter],self.FilterMaxSeeing[filter])
                    if self.log and self.verbose > 1:
                        self.log.info('TOSS: propID=%d field=%d  filter=%s  WeakLensingProp: suggestObs(): bad seeing:%f' % (
                            self.propID, fieldID, filter, filterSeeingList[filter]))
                    continue
                ...
                if GlobalNeedFactor > 0.0:
                    if FieldNeedFactor > 0.0:
                        ...
                        FilterNeedFactor = (
                            self.maxNeedAfterOverflow/(nVisits[filter]-self.GoalVisitsFieldFilter[filter]+1)) / self.GoalVisitsFieldFilter[filter]
                ...
                # print "WLRANK: f:%d am:%f see:%f brt:%f phs:%f dt:%d flt:%s rnk:%f" %
                # (fieldID,
                # airmass,filterSeeingList[filter],intargetProfiles[i][1],moonPhase, date,
                # filter, rank)

        if self.log and self.verbose > 0:
            self.log.info('%sProp: suggestObs() propID=%d : Fields received=%i invisible=%i moon=%i Field-Filters allowed=%i badseeing=%i proposed=%i' %
                          (self.propFullName, self.propID, fields_received, fields_invisible, fields_moon, ffilter_allowed, ffilter_badseeing, ffilter_proposed))
    ...
    def closeObservation(self, observation, obsHistID, twilightProfile):
        ...
        if obs != None:
            ...
            if (self.log and self.verbose > 0):
                ...
                self.log.info('WeakLensingProp: closeObservation() propID=%d field=%d filter=%s propRank=%.4f finRank=%.4f t=%dd%02dh%02dm%02ds progress=%d%%' % (
                    self.propID, obs.fieldID, obs.filter, obs.propRank, obs.finRank, t_days, t_hour, t_mins, t_secs, int(100*progress)))
