...
#        listOfNewFields=self.targetsNewSeq.keys()
        ...
        if (self.log):
            self.log.info('%sProp: startNight() propID=%d added %i new sequences, deleted %i notstarted, restarted %i lost, restarted %i complete, kept %i' % (
                self.propFullName, self.propID, newseq, notstarted, restartedlost, restartedcomplete, keptsequences))
    ...
    def GetProgressPerFilter(self):
        ...
        if (self.log):
            self.log.info('%sProp: GetProgressPerFilter() propID=%d Sequence progress: %.3f%%' %
                ...
                if (airmass > self.maxAirmass):
                    if self.log and self.verbose > 2:
                        self.log.info('%sProp: suggestObs() propID=%d field=%i too low:%f' %
                                      (self.TransientType, self.propID, fieldID, airmass))
                    fields_invisible += 1
                    continue
...
#                    for filter in rankForFilters.keys():
                    for filter in sorted(rankForFilters.iterkeys()):
                        ...
                        rank = rankTime*self.rankTimeMax + \
                            rankForFilters[filter]*self.rankFilter + rankLossRisk * \
                    ...
                    obsHist = self.lsstDB.addMissedObservation(
                        self.sequences[fieldID].GetNextFilter(), date, mjd, 0, lst_RAD, self.sessionID, fieldID)
                    ...
                    if self.log and self.verbose > 0 and not self.sequences[fieldID].IsLost():
                        ...
                        self.log.info('%sProp: suggestObs() event MISSED for propID=%d field=%i t=%dd%02dh%02dm%02ds progress=%i%%' % (
                            self.TransientType, self.propID, fieldID, t_days, t_hour, t_mins, t_secs, 100*self.sequences[fieldID].GetProgress()))
                    ...
                    if self.sequences[fieldID].IsLost():
                        if self.log and self.verbose > 0:
                            self.log.info('%sProp: suggestObs() sequence LOST for propID=%d field=%i t=%.0f event missed' % (
                                self.TransientType, self.propID, fieldID, date))
            ...
            if self.log and self.verbose > 0:
                self.log.info('%sProp: suggestObs() propID=%d fields count: received=%i invisible=%i moon=%i nottoday=%i waiting=%i proposed=%i missed=%i lost=%i completed=%i fieldfilter=%i' % (
                    self.TransientType, self.propID, fields_received, fields_invisible, fields_moon, fields_nottoday, fields_waiting, fields_proposed, fields_missed, fields_lost, fields_completed, fieldfilter_proposed))
...
#            self.reuseRanking = self.reuseRankingCount
    ...
    def closeObservation(self, observation, twilightProfile, obsHistID):
        ...
        if obs != None:
            ...
            if self.log and self.verbose > 0:
                ...
                self.log.info('%sProp: closeObservation() propID=%d field=%d filter=%s propRank=%.4f finRank=%.4f t=%dd%02dh%02dm%02ds progress=%d%%' % (
                    self.propFullName, self.propID, obs.fieldID, obs.filter, obs.propRank, obs.finRank, t_days, t_hour, t_mins, t_secs, int(100*progress)))
    ...
    def CompleteSequence(self, fieldID, date):
        ...
        if (self.log and self.verbose > 0):
            self.log.info('%sProp: closeObservation() sequence COMPLETE for propID=%d field=%d' %
    ...
    def FinishSequences(self, obsdate):
        ...
        for fieldID in self.sequences.keys():
            if self.sequences[fieldID].IsProgressing():
                if (self.log):
                    self.log.info('%sProp: FinishSequences() propID=%d sequence LOST for field=%i end of cycle' % (
                        self.propFullName, self.propID, fieldID))
    ...
    def closeProposal(self, time):
        ...
        for fieldID in self.sequences.keys():
            if self.sequences[fieldID].IsProgressing():
                if (self.log):
                    self.log.info('%sProp: closeProposal() propID=%d sequence LOST for field=%i end of simulation' % (
                        self.propFullName, self.propID, fieldID))
                self.sequences[fieldID].SetLost()
    ...
    def RestartSequences(self):
        ...
        for fieldID in self.sequences.keys():
            if self.sequences[fieldID].IsLost() or (self.sequences[fieldID].IsComplete() and not fieldID in self.blockedTargets):
                ...
                self.sequences[fieldID].Restart(self.SeqCount)
                if self.log and self.verbose > 0:
                    self.log.info('TransientProp: RestartSequences() sequence for propID=%d field=%i restarted' % (
                        self.propID, fieldID))
