...
def printstate(headstr, state):
    print("%s tracking=%5s TelAlt=%8.3f TelAz=%8.3f Rot=%8.3f" % (headstr, str(state.Tracking),
                                                                  state.TelAlt_Pos_RAD*RAD2DEG, state.TelAz_Pos_RAD*RAD2DEG, state.Rotator_Pos_RAD*RAD2DEG))
    print("%s DomAlt=%8.3f DomAz=%8.3f Filter=%s" %
          (headstr, state.DomAlt_Pos_RAD*RAD2DEG, state.DomAz_Pos_RAD*RAD2DEG, state.Filter_Pos))
    print("%s RA=%8.3f Dec=%8.3f Angle=%8.3f Time=%f" %
          (headstr, state.RA_RAD*RAD2DEG, state.DEC_RAD*RAD2DEG, state.ANG_RAD*RAD2DEG, state.TIME))
...
obsProfile = (longitude * DEG2RAD, latitude * DEG2RAD, height,
              simEpoch, pressure, temperature, relativeHumidity)
...
for k in range(len(RA_DEG)):
    ...
    print("RaDec2AltAz() ra=%8.3f dec=%8.3f time=%8.0f ==> alt=%8.3f az=%8.3f pa=%8.3f" %
          (ra_DEG, dec_DEG, time_SEC, alt_RAD*RAD2DEG, az_RAD*RAD2DEG, pa_RAD*RAD2DEG))
...
for j in range(len(start_angle_DEG)):
    for k in range(len(target_angle_DEG)):
        ...
        (distance_RAD, final_RAD) = test_state.GetShortestDistanceWithWrap(
            target_DEG*DEG2RAD, start_DEG*DEG2RAD, min_angle_DEG*DEG2RAD, max_angle_DEG*DEG2RAD)
        print("GetShortestDistanceWithWrap() start=%6.1f target=%6.1f ==> distance=%6.1f final=%6.1f" %
              (start_DEG, target_DEG, distance_RAD*RAD2DEG, final_RAD*RAD2DEG))
