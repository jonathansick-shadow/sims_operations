...
def printstate(headstr, state):
    print("%s tracking=%5s TelAlt=%7.2f TelAz=%7.2f Rot=%7.2f" % (headstr, str(state.Tracking),
                                                                  state.TelAlt_Pos_RAD*RAD2DEG, state.TelAz_Pos_RAD*RAD2DEG, state.Rotator_Pos_RAD*RAD2DEG))
    print("%s DomAlt=%7.2f DomAz=%7.2f Filter=%s" %
...
for k in range(len(ALT_DEG)):
    ...
    print("AltAz2RaDec() alt=%7.2f az=%7.2f time=%8.0f ==> ra=%8.3f dec=%8.3f" %
          (alt_DEG, az_DEG, time_SEC, ra_RAD*RAD2DEG, dec_RAD*RAD2DEG))
...
for k in range(len(RA_DEG)):
    ...
    print("RaDec2AltAz() ra=%8.3f dec=%8.3f time=%8.0f ==> alt=%7.2f az=%7.2f pa=%8.3f ha=%8.3f" %
          (ra_DEG, dec_DEG, time_SEC, alt_RAD*RAD2DEG, az_RAD*RAD2DEG, pa_RAD*RAD2DEG, ha_HOU))
